...
index.mu.spline.n	<-	which.max(dydt.spline$y)						#index at which maximum growth rate occurs
time.mu.spline.n	<-	dydt.spline$x[index.mu.spline.n]					#time at which maximum growth rate occurs.
mu.spline.n		<-	max(dydt.spline$y) 								#dydt.max	#same as dydt.spline$y[time.mu]? 


#Find Lag time (A)
y.mu.spline.n		<-	predict(gc.spline, x=time.mu.spline.n)[["y"]]			#Y location of maximum growth rate, found to be on spline fit
y.mu.diff.n 		<- y.mu.spline.n-gc.spline$y[1]
lambda.spline.n	<- 	time.mu.spline.n-(y.mu.diff.n/mu.spline.n)			#Lag time, x intercept of maximum slope.oo
...
mu.all <- gFindMu(gc.spline, dydt.spline, dydtdt.spline)
mu <- mu.all$mu
time.mu <- mu.all$time.mu
y.mu <- mu.all$y.mu
lambda <- mu.all$lambda
A <- mu.all$A
time.A <- mu.all$time.A
index.A <-mu.all$index.A

# Areas under curve
integral.spline <- 	low.integrate(gc.spline$x[1:index.A], gc.spline$y[1:index.A])			#Should we use low.integrate? Something simpler instead? low.integrate from grofit package.

#hill.fit.coefs <-gFitHill(x=gc.spline$x, y=gc.spline$y, control=control)
...
spline.fit <-list(version = version, raw.time = time, raw.data = data, deriv=dydt.spline, secderiv=dydtdt.spline, 
	...
	graph=list(time.mu=time.mu, time.A=time.A, y.mu=y.mu),#Get rid of this
	parameters=list(A=A, mu=mu, lambda=lambda, time.mu=time.mu, 
		time.A=time.A, y.mu=y.mu, initial.od=initial.od, integral=integral.spline, 
		...
		naive.mu=mu.spline.n, naive.lambda=lambda.spline.n, naive.time.mu=time.mu.spline.n, niave.y.mu=y.mu.spline.n
