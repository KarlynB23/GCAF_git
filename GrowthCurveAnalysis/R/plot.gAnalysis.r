...
quartz(width=unit(8, "inches"), height=unit(6, "inches")) # "windows" changed to "quartz" for MAC by ST #Change this?
#pdf(file=paste(control$export.folder, "/", "QuickGraph.pdf", sep=""), width=11, height=8.5)
grid.newpage()
...
# A
grid.segments(x0=unit(0, "native"), x1=g.analysis.well$parameters$time.A, y0=g.analysis.well$parameters$A, y1=g.analysis.well$parameters$A,
...
#dev.off() - only if using pdF instead
# # # # # # # # # # # # # # # # # # # # # For many well view # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
